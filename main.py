...
def allFeature(arr):
    ...
    t_value, sizeStatistic_result=allFeature_core(arr_32x32)
    # 计算 28% 的值并输出
    return t_value, sizeStatistic_result
...
def allFeatureNorm(arr):
    t_value, sizeStatistic_result=allFeature(arr);
    ...
    return t_value, sizeStatistic_result_norm

def allFeatureNorm_fine(arr):
    t_value, sizeStatistic_result=allFeature_core(arr);
    sizeStatistic_result_norm = np.zeros(24, float)
    for i in range(4):
        _sum=1
        for j in range(6):
